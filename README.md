...
| `signup.html` | 회원가입(보호자) — 보호자 이름/이메일 · 자녀 이름 · 자녀 출생연도(3~7세) · 국가 · 비밀번호 |
...
관리자 계정으로 로그인하면 헤더에 **관리자** 버튼이 나타나고, `admin.html`에서 공지·대회 CRUD와 회원 목록을 관리할 수 있습니다. 관리자 계정 정보는 별도 관리합니다.

> ⚠️ 프론트엔드 전용 프로토타입이라 인증 로직이 `assets/js/data.js` · `auth.js`에 있습니다. 실제 서비스로 전환할 때는 반드시 백엔드 인증(예: Supabase, Firebase)으로 교체하세요.
